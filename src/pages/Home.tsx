...
const getFontSizeClass = (name: string) => {
  if (!name) return "text-2xl sm:text-3xl md:text-[60px] lg:text-[90px] leading-[0.95]";
  const formatted = formatCakeName(name);
  const lines = formatted.split("\n");
  const longestLineLen = Math.max(...lines.map(l => l.length));

  if (longestLineLen > 25) {
    return "text-lg sm:text-xl md:text-[28px] lg:text-[40px] leading-[1.05]";
  } else if (longestLineLen > 18) {
    return "text-xl sm:text-2xl md:text-[36px] lg:text-[50px] leading-[1.0]";
  } else if (longestLineLen > 13) {
    return "text-2xl sm:text-3xl md:text-[44px] lg:text-[65px] leading-[0.95]";
  } else if (longestLineLen > 10) {
    return "text-3xl sm:text-4xl md:text-[56px] lg:text-[80px] leading-[0.9]";
  }
  return "text-4xl sm:text-5xl md:text-[68px] lg:text-[90px] leading-[0.9]";
};
...
export default function Home() {
  ...
  return (
    <div className="flex flex-col">
      ...
      {/* Hero Section */}
      <section className="relative lg:h-screen grid grid-cols-12 gap-0 overflow-hidden">
        <div className="col-span-12 lg:col-span-7 p-10 md:p-20 flex flex-col justify-start lg:justify-between relative z-10 text-center lg:text-left items-center lg:items-start">
          <div className="mb-8 md:mb-0">
            <motion.h1 
              initial={{ x: -20, opacity: 0 }}
              animate={{ x: 0, opacity: 1 }}
              className="text-[16vw] lg:text-[140px] leading-[0.8] font-black uppercase tracking-tighter mb-4 md:mb-8 italic break-words"
            >
              JACKY'S<br/><span className="text-amber-500">BAKERY</span>
            </motion.h1>
            <p className="text-base md:text-2xl leading-relaxed text-white/90 max-w-sm md:max-w-md font-light serif italic mx-auto lg:mx-0">
          ...
          {(() => {
            ...
            return (
              <motion.div 
                ...
                className="bg-amber-600 p-8 sm:p-12 md:p-16 lg:p-20 text-black transform -rotate-2 w-[95%] sm:w-[90%] lg:w-full max-w-md md:max-w-2xl lg:max-w-4xl mt-4 md:mt-12 mx-auto lg:mx-0 flex flex-col justify-center text-center lg:text-left min-h-[220px] md:min-h-[300px] shadow-2xl relative"
              >
                <div className="flex items-center justify-between mb-4 md:mb-6">
                  <span className="block text-[10px] md:text-base uppercase font-bold tracking-[0.2em] md:tracking-[0.5em] opacity-70">Cake of the Day</span>
                  {settings?.cakeOfTheDayPrice !== undefined && settings?.cakeOfTheDayPrice > 0 && (
                    <span className="font-mono text-xs sm:text-sm md:text-base font-bold bg-black text-amber-500 px-3 py-1 scale-95 tracking-wider skew-x-[-4deg] shadow-md uppercase">
                      {settings.cakeOfTheDayPrice.toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })}
                    </span>
                  )}
                </div>
                ...
                <p className="text-[10px] md:text-lg font-bold mt-4 md:mt-8 uppercase tracking-widest flex items-center justify-center lg:justify-start gap-2 opacity-90">
        ...
        <div className="hidden lg:block lg:col-span-5 relative border-l border-white/10">
          <img 
      ...
      <section className="py-24 md:py-40 px-6 md:px-10 bg-amber-500/[0.02]">
        <div className="max-w-7xl mx-auto flex flex-col items-center text-center">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-32 w-full text-center lg:text-left">
            <div className="space-y-12">
              <div className="block group">
                <p className="text-[10px] uppercase tracking-[0.3em] text-amber-500 mb-4 font-bold text-center lg:text-left">Direkt Anrufen</p>
                <div className="flex items-center justify-center lg:justify-start gap-4">
                  <a 
                    href="tel:+43676123456" 
                    className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-black group-hover:text-amber-500 transition-colors tracking-tighter tabular-nums break-words select-all max-w-full"
                  >
                    +43 676 123456
                  </a>
                </div>
              ...
              <div className="block group">
                <p className="text-[10px] uppercase tracking-[0.3em] text-amber-500 mb-4 font-bold text-center lg:text-left">Schreib mir</p>
                <div className="flex flex-col items-center lg:items-start gap-2">
                  <a 
                    href="mailto:beispiel@gmail.com" 
                    className="text-xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl font-black group-hover:text-amber-500 transition-colors tracking-tighter break-all select-all max-w-full"
                  >
                    beispiel@gmail.com
                  </a>
                </div>
            ...
            <div className="flex flex-col justify-center items-center lg:items-start space-y-6 lg:space-y-8">
              <div className="w-full max-w-sm">
                <p className="text-[10px] uppercase tracking-[0.3em] text-amber-500 mb-4 font-bold text-center lg:text-left">The Social Club</p>
                <div className="flex justify-center lg:justify-start">
                  <a href="#" className="flex items-center gap-4 text-2xl sm:text-3xl font-black uppercase tracking-tighter hover:text-amber-500 transition-colors">
                    Instagram <ArrowRight />
                  </a>
                </div>
              </div>
              <p className="text-sm serif italic opacity-90 max-w-xs text-center lg:text-left">
